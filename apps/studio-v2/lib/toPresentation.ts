    ...
    const slides: Slide[] = run.map((beat) => {
      ...
      const textLayer = makeLayer(
        fx?.effectType ?? 'basic-text',
        {
          ...
          // effects (basic-text).
          duration: holdSec,
          // The generator's per-beat Ken Burns call overrides the atom's own
          // setting; when the beat carries none, the atom's config stands.
          ...(beat.kenBurns ? { kenBurns: beat.kenBurns } : {}),
        },
